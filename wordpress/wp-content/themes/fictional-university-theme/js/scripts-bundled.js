...
var Like = function () {
  ...
  _createClass(Like, [{
    ...
    value: function createLike($likeBox) {
      ...
      // success
      function (likeId) {
        var likeCount = parseInt($likeBox.find('.like-count').text(), 10);
    ...
    key: 'deleteLike',
    value: function deleteLike($likeBox) {
      _jquery2.default.ajax({
        'url': UNIVERSITY_DATA.rootUrl + '/wp-json/university/v1/like',
        'type': 'DELETE',
        'data': {
...
var MyNotes = function () {
  ...
  _createClass(MyNotes, [{
    ...
    value: function makeNoteEditable($noteItem) {
      $noteItem.data('state', 'editable').find('.note-title-field, .note-body-field').removeAttr('readonly').addClass('note-active-field').end().find('.update-note').addClass('update-note--visible').end().find('.edit-note').html('<i class="fa fa-times" aria-hidden="true"></i> Cancel');
    }
  }, {
    key: 'makeNoteReadonly',
    value: function makeNoteReadonly($noteItem) {
      $noteItem.data('state', '').find('.note-title-field, .note-body-field').attr('readonly', 'readonly').removeClass('note-active-field').end().find('.update-note').removeClass('update-note--visible').end().find('.edit-note').html('<i class="fa fa-pencil" aria-hidden="true"></i> Edit');
    }
  }, {
    key: 'deleteNote',
